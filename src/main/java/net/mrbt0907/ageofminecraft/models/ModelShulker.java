...
public class ModelShulker extends ModelBase
{
	...
	public ModelShulker()
	{
		...
		this.head.setTextureOffset(0, 52).addBox(-3.0F, -6.0F, -3.0F, 6, 6, 6);
	...
	*/
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
	{
		...
		this.head.setRotationPoint(0.0F, 18.0F + f4, 0.0F);
		this.lid.setRotationPoint(0.0F, 16.0F + MathHelper.sin(f1) * 8.0F + f3, 0.0F);
		if (entityshulker.getJukeboxToDanceTo() != null)
		{
			this.lid.setRotationPoint(0.0F, 10.0F + MathHelper.sin(f1) * 8.0F + f3 + (MathHelper.cos(ageInTicks * 0.5F) * 4F), 0.0F + (MathHelper.cos(ageInTicks * 0.25F) * 2F));
			this.head.setRotationPoint(0.0F, 18.0F + f4 + (MathHelper.cos(ageInTicks * 0.5F) * 1F), 0.0F + (MathHelper.cos(ageInTicks * 0.25F) * 1F));
		}if (!entityshulker.getCurrentBook().isEmpty())
			{
				this.head.setRotationPoint(0.0F, 16.0F + f4, 0.0F);
			}
