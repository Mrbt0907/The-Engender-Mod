...
public class SpawnerRegistry 
{
	public static final BiConsumer<EntityPlayer, Object[]> SPAWN_NORMAL = new BiConsumer<EntityPlayer, Object[]>()
	{
		...
		public void accept(EntityPlayer player, Object[] args)
		{
			...
			EntityEngendered entity;
			
			for (int i = 0; i < amount; i++)
			{
				try
				{
					entity = (EntityEngendered) ((Class<?>)args[1]).getConstructor(World.class).newInstance(player.world);
				}
				catch (Exception e)
				{
				...
				if (args[7] != null)
					((Consumer<EntityEngendered>)args[7]).accept(entity);
