	...
	public void attackEntityWithRangedAttack(EntityLivingBase target, float p_82196_2_)
	{
		for (int i = 0; i < (this.isHero() ? 3 : 1); i++)
		{
			if (!func_184730_o() && this.canEntityBeSeen(target))
			{
				...
				else if (target.isNonBoss() && (target.getHealth() >= 2.0F) && (!target.isEntityUndead()) && target.isPotionApplicable(new PotionEffect(MobEffects.POISON)) && (!(target instanceof net.mrbt0907.ageofminecraft.entity.tier3.EntitySpider)) && (!(target instanceof net.minecraft.entity.monster.EntitySpider)) && (!target.isPotionActive(MobEffects.POISON)) && !this.isOnSameTeam(target))
				{
