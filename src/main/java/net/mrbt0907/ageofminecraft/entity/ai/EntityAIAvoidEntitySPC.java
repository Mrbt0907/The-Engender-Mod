...
public class EntityAIAvoidEntitySPC<T extends Entity> extends EntityAIBase
{
	...
	public EntityAIAvoidEntitySPC(EntityCreature theEntityIn, Class<T> classToAvoidIn, Predicate <? super T > avoidTargetSelectorIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn)
	{
		this.canBeSeenSelector = new Predicate<Entity>()
		{
			...
			{
				return EntityAIAvoidEntitySPC.this.theEntity.getEntitySenses().canSee(p_apply_1_) && !EntityAIAvoidEntitySPC.this.theEntity.isOnSameTeam(p_apply_1_);
			}
		};
	...
	*/
	...
	{
		List<T> list = this.theEntity.world.<T>getEntitiesWithinAABB(this.classToAvoid, this.theEntity.getEntityBoundingBox().grow((double)this.avoidDistance, (double)this.avoidDistance, (double)this.avoidDistance), Predicates.and(new Predicate[] {EntitySelectors.CAN_AI_TARGET, this.canBeSeenSelector, this.avoidTargetSelector}));
		...
		else
		{
			...
			else if (!this.entityPathNavigate.noPath())
			{
				return false;
			}
			else if (this.closestLivingEntity.isDead)
			{
				return false;
			}
