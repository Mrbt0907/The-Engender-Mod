package net.mrbt0907.ageofminecraft.entity.tier5.dragonphases;
...
{
	private static final DataParameter<Float> RADIUS = EntityDataManager.<Float>createKey(EntityAreaEffectCloudOther.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> COLOR = EntityDataManager.<Integer>createKey(EntityAreaEffectCloudOther.class, DataSerializers.VARINT);
	private static final DataParameter<Boolean> IGNORE_RADIUS = EntityDataManager.<Boolean>createKey(EntityAreaEffectCloudOther.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> PARTICLE = EntityDataManager.<Integer>createKey(EntityAreaEffectCloudOther.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> PARTICLE_PARAM_1 = EntityDataManager.<Integer>createKey(EntityAreaEffectCloudOther.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> PARTICLE_PARAM_2 = EntityDataManager.<Integer>createKey(EntityAreaEffectCloudOther.class, DataSerializers.VARINT);
	...
	public void onUpdate()
	{
		...
		else
		{
			...
			if (this.ticksExisted % 20 == 0)
			{
				...
				{
					List<EntityLivingBase> list = this.world.<EntityLivingBase>getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().grow(1D));
								...
								if (d2 <= (double)(f * f))
								{
									...
									else
									{
										for (PotionEffect potioneffect : potions)
										{
											if (potioneffect.getPotion().isInstant())
											{
												this.getOwner().inflictEngenderMobDamage(entitylivingbase, " was killed by magic created by ", new EntityDamageSource("indirectMagic", this.getOwner()).setMagicDamage().setDamageBypassesArmor().setDamageIsAbsolute(), 8F);
											}
			...
			public boolean processInitialInteract(EntityPlayer player, EnumHand hand)
			{
				...
				if (!this.world.isRemote && !stack.isEmpty() && stack.getItem() == Items.GLASS_BOTTLE)
				{
					...
					world.playSound((EntityPlayer)null, player.posX, player.posY, player.posZ, SoundEvents.ITEM_BOTTLE_FILL_DRAGONBREATH, SoundCategory.NEUTRAL, 1.0F, 1.0F);
					this.turnBottleIntoItem(stack, player, new ItemStack(Items.DRAGON_BREATH));
