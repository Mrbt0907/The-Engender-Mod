package net.mrbt0907.ageofminecraft.effects;
...
public class EngenderExplosion extends Explosion
{
	...
	public void doExplosionA()
	{
		if (this.explosionSize > 0)
		{
			...
			for (int k2 = 0; k2 < list.size(); ++k2)
			{
				...
				if (entity != null && !entity.isImmuneToExplosions() && !(entity instanceof EntityFireball))
				{
					double d12 = entity.getDistance(this.explosionX, this.explosionY, this.explosionZ) / ((double)f3 * 4D);
					...
					if (d12 <= 1.0D)
					{
						...
						if (entity.getDistance(this.explosionX, this.explosionY, this.explosionZ) <= 3D)
						d7 = entity.posY;
						...
						if (d13 != 0.0D)
						{
							...
							if (exploder instanceof EntityFriendlyCreature && entity instanceof EntityLivingBase && !exploder.isOnSameTeam((EntityLivingBase)entity))
							((EntityFriendlyCreature)exploder).inflictEngenderMobDamage((EntityLivingBase)entity, " was blown up by ", DamageSource.causeExplosionDamage(this), (float)((int)((d10 * d10 + d10) / 2.0D * 7.0D * (double)f3 + 1.0D)));
							double d11 = d10 * f3;
							...
							if (!(entity instanceof IEntityMultiPart) && entity instanceof EntityLivingBase && !exploder.isOnSameTeam((EntityLivingBase)entity))
							{
								entity.motionX += d5 * d11;
								entity.motionY += d7 * d11 + 1D;
	...
	*/
	public void doExplosionB(boolean spawnParticles)
	{
		...
		if (this.isSmoking && this.explosionSize > 0)
		{
			for (BlockPos blockpos : this.affectedBlockPositions)
			{
				...
				{
					((WorldServer) world).spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, (d0 + this.explosionX) / 2.0D, (d1 + this.explosionY) / 2.0D, (d2 + this.explosionZ) / 2.0D, d3, d4, d5, new int[0]);
					((WorldServer) world).spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d0, d1, d2, d3, d4, d5, new int[0]);
				}
