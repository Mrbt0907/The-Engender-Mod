	...
	*/
	...
	public void onUpdate()
	{
		...
		if (this.ticksExisted > 4 && this.shootingEntity != null && targetEntity != null)
		{
			...
			if (!this.world.isRemote)
			if (targetEntity instanceof EntityLiving && !(targetEntity instanceof EntityFriendlyCreature))
			{
				((EntityFriendlyCreature)this.shootingEntity).inflictEngenderMobDamage((EntityLivingBase)targetEntity, " was vaporized by ", new EntityDamageSource("indirectMagic", shootingEntity).setMagicDamage().setDamageBypassesArmor().setDamageIsAbsolute(), 20F);
				if (((EntityLiving)targetEntity).getHealth() <= 80F && targetEntity.isNonBoss())
				targetEntity.setDead();
				targetEntity.playSound(SoundEvents.ENTITY_GENERIC_EXPLODE, 2.0F, 2.0F);
				targetEntity.playSound(SoundEvents.BLOCK_LAVA_EXTINGUISH, 2.0F, 2.0F);
				targetEntity.playSound(SoundEvents.BLOCK_LAVA_POP, 2.0F, 2.0F);
				targetEntity.world.setEntityState(targetEntity, (byte)20);
			}
			else
			{
				((EntityFriendlyCreature)this.shootingEntity).inflictEngenderMobDamage((EntityLivingBase)targetEntity, " was vaporized by ", new EntityDamageSource("indirectMagic", shootingEntity).setMagicDamage().setDamageBypassesArmor().setDamageIsAbsolute(), 20F);
