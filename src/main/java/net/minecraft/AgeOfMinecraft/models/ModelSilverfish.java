...
public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entity)
{
	...
	for (int i = 0; i < this.silverfishBodyParts.length; ++i)
	{
		this.silverfishBodyParts[i].rotateAngleY = MathHelper.cos((silverfish.isAIDisabled() ? 1 : ageInTicks) * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * (silverfish.isSneaking() ? 0.0125F : 0.05F) * (float)(1 + Math.abs(i - 2));
		this.silverfishBodyParts[i].rotationPointX = MathHelper.sin((silverfish.isAIDisabled() ? 1 : ageInTicks) * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * (silverfish.isSneaking() ? 0.05F : 0.2F) * (float)Math.abs(i - 2);
	}
	if (silverfish.getJukeboxToDanceTo() != null)
	{
		for (int i = 0; i < this.silverfishBodyParts.length; ++i)
		{
			this.silverfishBodyParts[i].rotateAngleY = MathHelper.cos((silverfish.isAIDisabled() ? 1 : ageInTicks) * 0.45F + (float)i * 0.125F * (float)Math.PI) * (float)Math.PI * 0.1F * (float)(1 + Math.abs(i - 2));
			this.silverfishBodyParts[i].rotationPointX = MathHelper.sin((silverfish.isAIDisabled() ? 1 : ageInTicks) * 0.45F + (float)i * 0.125F * (float)Math.PI) * (float)Math.PI * 0.2F * (float)Math.abs(i - 2);
		}
	}
